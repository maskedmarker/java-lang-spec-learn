...
This class and its nested classes provide the main functionality and control for a set of worker threads:
Submissions from non-FJ threads enter into submission queues. (外部线程提交的任务都被提交了提交队列)
Workers take these tasks and typically split them into subtasks that may be stolen by other workers. (工作线程从提交队列把外部提交的任务偷走并执行,在执行任务时会产生子任务,这些子任务不会被放到提交队列而是放到各个工作线程的本地任务队列.不仅有提交队列而且本地任务队列的任务也会被其他工作线程偷走) 
(注意:提交队列使用的是偶数索引值,本地任务队列使用的是奇数索引值)


Preference rules give first priority to processing tasks from their own queues (LIFO or FIFO, depending on mode), then to randomized FIFO steals of tasks in other queues.  (config可以控制以LIFO或FIFO顺序执行本地任务队列)
This framework began as vehicle for supporting tree-structured parallelism using work-stealing.    (任务产生子任务,这样产生一棵任务树.同时work-stealing的负载均衡策略避免cpu有core空闲,提供更好的并发性能)
```
...
These are special forms of Deques that support only three of the four possible end-operations -- push, pop, and poll (aka steal), under the further constraints that push and pop are called only from the owning thread (or, as
extended here, under a lock), while poll may be called from other threads.  (If you are unfamiliar with them, you probably
...
Because we rely on CASes of references, we do not need tag bits on base or top.  They are simple ints as used in any circular array-based queue (see for example ArrayDeque).  
Updates to the indices guarantee that top == base means the queue is empty, but otherwise may err on the side of possibly making the queue
...
the aggregate, we ensure at least probabilistic non-blockingness.  If an attempted steal fails, a thief always chooses a different random victim target to try next. So, in
order for one thief to progress, it suffices for any in-progress poll or new push on any empty queue to
complete. (This is why we normally use method pollAt and its variants that try once at the apparent base index, else consider alternative actions, rather than method poll, which retries.)
...
WorkQueues are also used in a similar way for tasks submitted to the pool. We cannot mix these tasks in the same queues used by workers. 
Instead, we randomly associate submission queues with submitting threads, using a form of hashing.  
