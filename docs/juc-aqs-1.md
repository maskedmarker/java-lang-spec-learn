...
后面节点插入FIFO队列时,根据需要设置前节点的waitStatus,用来表示前节点释放锁资源时,需要通知后节点线程.
...
说明：
初始为 0.
一般是在未进入等待状态前,或状态被清除后.(表示中性,无特殊意义)
```
...
```text
同步队列是从尾部增长的.(即tail往前遍历,然后追加的)
释放锁资源时,需要通知后续节点的过程中,如果担心队列发生了变化,可以从tail往前查找.
```